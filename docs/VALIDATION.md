...
- A live headless Chromium interaction run exercised desktop Ctrl/Cmd toggles, inclusive Shift ranges, left-side drag initiation, a four-item group drag after forced pointer-capture loss, unblurred drop hit-testing, atomic assignment, and the **Assign selected...** dialog. A 390 x 844 touch run exercised checkbox selection, real tray scrolling, press-and-hold activation, category targeting, and group drop; a captured layout pass confirmed the selection bar and bubbles fit without horizontal overflow.
...
- Focused manual-account coverage creates a Cash Safe with a starting balance, verifies owner-only creation and blank-payee defaults, proves transfer legs are inverse, linked, visible, and absent from unassigned/rule/Analytics work, rejects invalid transfers without partial writes, and verifies pair-wide deletion and restoration return both balances exactly.
...
- Live Chromium cross-month acceptance dropped a July 15 `PHILO TV` income transaction into August's **Other Income**, confirmed the August-target tray copy and drop highlight, and persisted an August 15 budgeting date and `$160` August activity with zero July category activity. The Transactions list used `PHILO TV` as its headline, and the run produced no runtime exception or horizontal overflow.
...
10. In the transaction inbox, check bubbles on desktop and touch, exercise Ctrl/Cmd toggling and an inclusive Shift range on desktop, then confirm dragging a checked bubble assigns the complete group while dragging an unchecked bubble assigns only that transaction. Open a transaction on touch and confirm its close button receives focus without opening the keyboard; tapping an editable field should still open it. After several successful drops, confirm each assignment plays one short pop, briefly swells and settles the destination category, creates no bottom confirmation banner, returns the tray without a snap, keeps it open, and focuses the next available transaction.
...
13. While viewing one month, assign a transaction imported in another month and confirm its budgeting date moves into the displayed month, retains or month-end clamps its day, and cannot be reverted by a later source sync. Confirm a failed group assignment changes no transactions and still presents a visible error.
